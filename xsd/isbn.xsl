...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:i="http://www.publishing.org"
                version="1.0">
        ...
        <xsl:choose>
            ...
            <xsl:when test="substring($portionOfISBNleftToProcess, 1, 1) = ' '">
                <xsl:call-template name="calculateCorrectCheckDigit">
                    ...
                    <xsl:with-param name="multiplicationFactor"
                            select="$multiplicationFactor"/>
                    <xsl:with-param name="portionOfISBNleftToProcess"
                            select="substring($portionOfISBNleftToProcess, 2)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="substring($portionOfISBNleftToProcess, 1, 1) = '-'">
                <xsl:call-template name="calculateCorrectCheckDigit">
